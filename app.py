...
def get_category(filepath):
    """
    Robustly extract the category (class folder name) from a path.
    The dataset structure is:
        .../Garbage Classification/<category>/<image_file>
    Strategy (tried in order):
      1. Walk up the path parts looking for a part that matches a known
         TrashNet category name (case-insensitive, ignoring trailing
         parenthetical suffixes like "(1)").
      2. Fall back to parent.name (original behaviour).
    """
    KNOWN_CATEGORIES = {
        'cardboard', 'glass', 'metal', 'paper', 'plastic', 'trash',
        # add any extra categories your dataset has here
    }
    p = Path(filepath)
    parts = p.parts  # e.g. ('C:\\', 'Users', ..., 'cardboard', 'image001.jpg')
    # Walk from the leaf upward; skip the filename itself (index -1)
    for part in reversed(parts[:-1]):
        # Strip trailing parenthetical "(n)" e.g. "cardboard(1)" -> "cardboard"
        clean = part.split('(')[0].strip().lower()
        if clean in KNOWN_CATEGORIES:
            return clean
    # Fallback: use the immediate parent folder name, stripped and lowercased
    return p.parent.name.split('(')[0].strip().lower()
# ═══════════════════════════════════════════════════════════════════
# INDEX LOADING
# ═══════════════════════════════════════════════════════════════════
...
def load_index(path):
    ...
    # Use the robust get_category() so we correctly parse paths like
    #   .../cardboard(1)/image001.jpg  →  "cardboard"
    categories = {}
    for p in index.keys():
        cat = get_category(p)
        categories.setdefault(cat, []).append(p)
    # ── Warn if something looks wrong ──────────────────────────────
    # (visible in the Streamlit terminal, not in the UI)
    if not categories:
        print("[WARN] categories dict is empty — check index paths.")
    else:
        print(f"[INFO] Loaded {len(index)} images across "
              f"{len(categories)} categories: {sorted(categories.keys())}")
    return index, norm_params, categories
def read_rgb(path):
    """
    Read an image and return it as RGB numpy array.
    Handles Windows paths with Unicode characters via imdecode fallback.
    """
    img = cv2.imread(str(path))
    if img is None:
        # Fallback for paths with non-ASCII / Unicode characters (Windows)
        try:
...
def read_bgr(path):
    """
    Read an image and return it as BGR numpy array (for feature extraction).
    Handles Windows paths with Unicode characters via imdecode fallback.
    """
    img = cv2.imread(str(path))
    if img is None:
...
# ── Warn in UI if categories are still empty ───────────────────────
if not all_categories:
    st.warning(
        '⚠️ **No categories could be parsed from the index paths.**\n\n'
        'The index was loaded but category names could not be extracted. '
        'Check that your index paths contain a recognised category folder '
        '(cardboard, glass, metal, paper, plastic, trash). '
        'Add extra names to `KNOWN_CATEGORIES` in `get_category()` if needed.\n\n'
        f'**Example path from index:** `{next(iter(index.keys()), "N/A")}`'
    )
# ── Session state ──────────────────────────────────────
defaults = {
    'query_bgr': None,
...
with tab_search:
    ...
    with col_left:
        ...
        # Only offer "Pick from dataset" if categories were parsed successfully
        source_options = ['Upload your own', 'Take a photo']
        if all_categories:
            source_options.append('Pick from dataset')
        source = st.radio(
            'Query source',
            source_options,
            horizontal=True,
        ...
        if source == 'Upload your own':
            ...
            if st.session_state.query_source != f'dataset:{picked_path}':
                # Use robust reader for Unicode/non-ASCII Windows paths
                img_bgr = read_bgr(picked_path)
                ...
                for col_idx in range(n_cols):
                    ...
                    with cols[col_idx]:
                        if img is not None:
                            ...
                            st.warning(f'Cannot load image #{i+1}')
...
with tab_stats:
    ...
    if cat_counts:
        ...
        st.warning('Category breakdown unavailable — paths could not be parsed.')
    st.markdown('---')
    st.subheader('Index path diagnostics')
    with st.expander('Show first 10 index paths (for debugging)'):
        sample_paths = list(index.keys())[:10]
        for sp in sample_paths:
            st.code(sp)
        st.caption(
            'The category is extracted from the folder immediately containing '
            'each image file. Expected pattern: `.../Garbage Classification/<category>/<file>`'
        )
    st.markdown('---')
